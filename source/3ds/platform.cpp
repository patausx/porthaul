...
#ifndef CLASSIC
[[maybe_unused]] void drawBubbles (ImDrawList *const drawList_, std::span<Bubble> const bubbles_, float const dt_)
{
	auto const &io = ImGui::GetIO ();
...
void drawStatus ()
{
	...
	// top-right cluster: just the clock (freespace lives in the body now, not crammed here)
	auto const size = ImGui::CalcTextSize (timeBuffer);
	auto const p5   = ImVec2 (p3.x - size.x - style.FramePadding.x, style.FramePadding.y);
	ImGui::GetForegroundDrawList ()->AddText (p5, ImGui::GetColorU32 (ImGuiCol_Text), timeBuffer);
#endif
}
}
...
bool platform::init ()
{
	...
	// NOTE: style (colors + geometry + 0.5 scale for the small screen) is applied
	// by ph::theme::apply() from main(), AFTER this init. Don't set style here.
	(void)style;
#endif
...
{
	// PORTHAUL: dropped the citro3d demo logo + floating bubbles (ftpd aesthetic).
	// background is solid INK now; identity comes from the UI itself.
	drawStatus ();
