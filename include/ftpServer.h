...
private:
	...
	/// \brief Draw the connect QR inline (for the CONNECT tab)
	void drawConnectInline ();

	/// \brief Draw the FTP client UI inline (for the CLIENT tab)
	void drawClientInline ();
#endif
	...
	/// \brief Active top-screen tab (0=SERVER 1=CLIENT 2=CONNECT), cycled by L/R
	int m_tab = 0;

	/// \brief Local file browser (bottom screen)
	ph::FileBrowser m_browser;

	/// \brief Remote FTP client (CLIENT tab)
	ph::FtpClient m_client;

	/// \brief CLIENT tab input buffers (soft keyboard fills via InputText)
	char m_clientHost[128] = {0};
	char m_clientUser[64]  = {0};
	char m_clientPass[64]  = {0};
	std::uint16_t m_clientPort = 21;

	/// \brief Selected remote entry index (-1 = none)
	int m_clientSelected = -1;

	/// \brief Previous client status, for detecting completion (toast edge)
	int m_clientPrevStatus = 0;
